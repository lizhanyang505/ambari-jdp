...
def spark_service(name, upgrade_type=None, action=None):
  ...
  if action == 'start':
    ...
    if name == 'jobhistoryserver' and effective_version and check_stack_feature(StackFeature.SPARK_16PLUS, effective_version):
      # create & copy spark2-prophet-yarn-archive.tar.gz to hdfs
      ...
      # create & copy spark2-prophet-hive-archive.tar.gz to hdfs
      if not params.sysprep_skip_copy_tarballs_hdfs:
    ...
    # Spark 1.3.1.2.3, and higher, which was included in PROPHET 2.3, does not have a dependency on Tez, so it does not
